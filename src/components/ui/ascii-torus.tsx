...
export default function AsciiTorus({ 
  ...
  useEffect(() => {
    ...
    const generateTorus = (R: number, r: number, stepTheta: number, stepPhi: number) => {
      const pts = [];
      for(let theta=0; theta<Math.PI*2; theta+=stepTheta) {
        for(let phi=0; phi<Math.PI*2; phi+=stepPhi) {
          ...
          pts.push({
            ...
            y: (R + r * cosT) * sinP,
            z: r * sinT,
            nx: cosT * cosP, ny: cosT * sinP, nz: sinT
          });
        }
      }
      return pts;
    };
    const pts = generateTorus(2.2, 1.0, 0.08, 0.04);
    ...
    function render() {
      ...
      if (ctx) {
        ...
        const cosB = Math.cos(t * 0.3), sinB = Math.sin(t * 0.3);

        const scale = Math.min(cols, rows) * 1.0;
        ...
        for(let i=0; i<pts.length; i++) {
          ...
          if(px >= 0 && px < cols && py >= 0 && py < rows) {
            ...
            if(zinv > zBuffer[idx]) {
              ...
              lumBuffer[idx] = L;
              if(L > -0.2) {
                const charIdx = Math.floor((L + 0.2) / 1.2 * (density.length - 1));
                charBuffer[idx] = density[Math.max(0, Math.min(density.length - 1, charIdx))];
              } else {
                charBuffer[idx] = ' ';
              }
            }
        ...
        for(let j=0; j<rows; j++) {
          for(let i=0; i<cols; i++) {
            ...
            if(c !== ' ') {
              ...
              ctx.fillText(c, i * spacing + spacing/2, j * spacing + spacing/2);
            }
          }
        }
        ctx.globalAlpha = 1.0;
  ...
  return (
    <div className={`relative w-full h-full flex items-center justify-center ${className}`} style={{ opacity }}>
